...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  switch (req.method) {
    case 'GET':
      return getRaceById();
    case 'PUT':
      return updateRaceById();
    case 'DELETE':
      return deleteRaceById();
    default:
      return res.status(405).json({ message: `Method ${req.method} not allowed` });
  }

  async function getRaceById() {
    const data = await prisma.race.findFirst(convertQueryToPrismaUtil(req.query, 'race'));
    return res.status(200).json(data);
  }

  async function updateRaceById() {
